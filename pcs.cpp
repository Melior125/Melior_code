...
void fr_commit_worker(G1*& Tk, G1*& g, Fr*& w, int rownum, int colnum)
{
    int idx;
    while (true)
    {
        ...
        for(int i = 0; i < colnum; i++)
        {
            // 每行做一次：Tk[idx] += g[i] * w[i*rownum + idx]
            G1 tmp = g[i] * w[(size_t)i * rownum + idx];
            Tk[idx] += tmp;
        }
        // 记录完成
        endq.Push(idx);
...
G1* prover_commit_Fr(Fr* w, G1* g, int l, int thread_n)
{
    int halfl  = l / 2;             
    int rownum = 1 << halfl;        // 2^(l/2)
    int colnum = 1 << (l - halfl);  // 2^(l - halfl)
...
void openCommit(std::vector<Fr> w,Fr*r,Fr eval,G1&G,G1*g,G1*comm,int l)
{
    int halfl=l/2;
    int rownum=(1<<halfl),colnum=(1<<(l-halfl));
    timer verf;
